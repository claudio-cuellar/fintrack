...
export class AuthService {
  ...
  login(email: string, password: string): Observable<AuthResponse> { return this.http.post<AuthResponse>(apiPath('auth/login'), { email, password }, { withCredentials: true }).pipe(tap((response) => this.store(response.data))); }
  register(input: { email: string; password: string; firstName: string; lastName: string }): Observable<AuthResponse> { return this.http.post<AuthResponse>(apiPath('auth/register'), input, { withCredentials: true }).pipe(tap((response) => this.store(response.data))); }
  logout(): void { this.http.post(apiPath('auth/logout'), {}, { withCredentials: true }).subscribe({ complete: () => this.clear() }); this.clear(); }
